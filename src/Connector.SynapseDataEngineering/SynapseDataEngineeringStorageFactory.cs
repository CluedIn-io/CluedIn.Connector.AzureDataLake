...
public class SynapseDataEngineeringStorageFactory : StorageFactoryBase, IStorageFactory
{
    ...
    {
        if (configuration is not SynapseDataEngineeringConnectorConfiguration castedJobData)
        {
            throw new ApplicationException($"Provided job data is not of expected type '{typeof(SynapseDataEngineeringConnectorConfiguration)}'. It is '{configuration.GetType()}'.");
        }
