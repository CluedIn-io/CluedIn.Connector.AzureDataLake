            ...
            foreach (var orgId in organizationIds)
            {
                             ...
                             x.ProviderId == new AzureDataLakeConstants().ProviderId))
                {
                    foreach (var stream in streams.Where(s => s.ConnectorProviderDefinitionId == provider.Id))
                    {
                        ...
                        var connection = await AuthenticationDetailsHelper.GetAuthenticationDetails(executionContext, providerDefinitionId);
                        var configurations = new AzureDataLakeConnectorJobData(
                            connection.Authentication.ToDictionary(authDetail => authDetail.Key, x => x.Value), containerName);
