...
internal class StorageScheduler : Scheduler
{
    private readonly IStorageConfigurationConstants _configurationConstants;
    private readonly IStorageFactory _fileStorageFactory;
    ...
    private async Task StreamJobProducerAsync(IScheduledJobQueue jobQueue)
    {
        ...
        await StreamsHelper.ForEachStreamAsync(_applicationContext, _configurationConstants, addStreamExportJobs);
        async Task addStreamExportJobs(ExecutionContext executionContext, ProviderDefinition definition, StreamModel stream)
        {
            ...
            var jobSchedule = await _fileStorageFactory.GetScheduleAsync(executionContext, stream);
