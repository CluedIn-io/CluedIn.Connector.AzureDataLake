...
internal abstract class StorageExportEntitiesJobBase : StorageJobBase
{
    ...
    protected StorageExportEntitiesJobBase(
        ...
        IStorageFactory dataLakeJobDataFactory,
        IDateTimeOffsetProvider dateTimeOffsetProvider) : base(appContext, dateTimeOffsetProvider)
    {
        ...
        _storageFactory = dataLakeJobDataFactory ?? throw new ArgumentNullException(nameof(dataLakeJobDataFactory));
    ...
    private async Task<ExportJobData> GetJobDataAsync(ExecutionContext context, IStorageJobArgs args, string taskName)
    {
        using var exportJobLoggingScope = context.Log.BeginScope(CreateLoggingScope(args));
    ...
    private DateTimeOffset GetAsOfTime(ExecutionContext context, IStorageJobArgs args, IStorageConfiguration jobData)
    {
