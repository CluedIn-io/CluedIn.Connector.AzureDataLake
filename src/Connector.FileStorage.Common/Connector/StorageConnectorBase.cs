...
namespace CluedIn.Connector.FileStorage.Common.Connector
{
    public abstract partial class StorageConnectorBase : ConnectorBaseV2
    {
        ...
        {
            if (streamModel.Mode != StreamMode.Sync || !configuration.IsStreamCacheEnabled)
            {
                return false;
            }

            return configuration.IsDeltaMode || configuration.IsSoftDelete;
        }
            ...
            bool useSoftDelete)
        {
            ...
            async Task InsertWithSoftDelete()
            {
                ...
                var changeTypeConstraint = syncItem.ChangeType == VersionChangeType.Changed
                    ? $"AND ([{StorageConfigurationConstants.ChangeTypeKey}] != '{nameof(VersionChangeType.Removed)}' OR [{StorageConfigurationConstants.ChangeTypeKey}] IS NULL)"
                    : string.Empty;
                ...
                var insertOrUpdateSql = $"""
                                         ...
                                         BEGIN
                                             ...
                                             WHERE
                                                 ...
                                                 [{StorageConfigurationConstants.PersistVersionKey}] < @EntityPersistVersion
        ...
        private async Task Flush(Partition partition, string[] entityData)
        {
            ...
            var configuration = partition.Configuration;
