...
public class AzureAIStudioStorageFactory : StorageFactoryBase, IStorageFactory
{
    ...
    {
        if (configuration is not AzureAIStudioConnectorConfiguration castedConfiguration)
        {
            throw new ApplicationException($"Provided job data is not of expected type '{typeof(AzureAIStudioConnectorConfiguration)}'. It is '{configuration.GetType()}'.");
        }
