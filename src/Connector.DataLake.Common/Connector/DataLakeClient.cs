...
public abstract class DataLakeClient : IDataLakeClient
{
    ...
    public Task<FileMetadata> GetFilePathProperties(IDataLakeJobData configuration, string fileName)
    {
        return GetFilePathProperties(configuration, fileName, string.Empty);
    }

    public async Task<FileMetadata> GetFilePathProperties(IDataLakeJobData configuration, string fileName, string subDirectory)
    {
        var fileSystemClient = await GetFileSystemClientAsync(configuration, ensureExists: false);
