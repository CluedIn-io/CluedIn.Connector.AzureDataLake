...
function Run-Setup() {
	...
	$dumpFiles = Get-ChildItem -Path "./test/integration/Connector.AmazonS3.Tests.Integration/" -Recurse | where { $_.Name -like "*.dmp" }
	Write-Host "WAAAAAAAAAAAAAAAA AmazonS3 totalDumpFIles = $($dumpFiles.Length)"
