...
function Run-Setup() {
	...
	Set-Variable "INTEGRATIONTEST_STREAMCACHE" $connectionStringEncoded
	...
	Get-Location
	dotnet test --blame-hang-timeout 1m --blame --blame-hang-dump-type full --diag log.txt ./test/integration/Connector.AmazonS3.Tests.Integration/Connector.AmazonS3.Tests.Integration.csproj --configuration Release# --no-restore --no-build
